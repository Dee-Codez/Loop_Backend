...
def home():
    try:
        ...
        cur = conn.cursor()
        cur.execute("SELECT * FROM timezones OFFSET 1010; ")
        res1 = cur.fetchall()
        for row in res1:
            store_id = row[0]
            timezone_str = row[1]
            ...
            data = {"store_id": store_id, "timezone": timezone_str, "open_time": open_times_str, "close_time": close_times_str,"hour_uptime":hour_uptime,"hour_downtime":hour_downtime ,"day_uptime":day_uptime,"day_downtime":day_downtime ,"hr_data": hourres, "day_data": dayres,"week_data":weekres}
